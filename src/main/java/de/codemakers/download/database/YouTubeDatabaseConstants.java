...
public class YouTubeDatabaseConstants {
     ...
     */
    @Deprecated
    public static final String QUERY_TABLE_VIDEO_QUEUE_SELECT_ALL_NEXT = String.format("SELECT * FROM %s WHERE %s = '%s' ORDER BY %s DESC, %s, %s;", IDENTIFIER_TABLE_VIDEO_QUEUE, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_STATE, QueuedVideoState.QUEUED.name(), IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_PRIORITY, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_REQUESTED, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_ID);
    /**
     * No arguments
     */
    @Deprecated
    public static final String QUERY_TABLE_VIDEO_QUEUE_SELECT_NEXT = String.format("SELECT * FROM %s WHERE %s = '%s' ORDER BY %s DESC, %s, %s LIMIT 1;", IDENTIFIER_TABLE_VIDEO_QUEUE, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_STATE, QueuedVideoState.QUEUED.name(), IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_PRIORITY, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_REQUESTED, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_ID); //FIXME The QUEUED State is at the wrong position in this list of Strings?! Same goes for the String above "QUERY_TABLE_VIDEO_QUEUE_SELECT_ALL_NEXT" //And check if this commit is properly signed and recognized by Gitea!
    /**
     * No arguments
     */
    public static final String QUERY_TABLE_VIDEO_QUEUE_SELECT_NEXT_AND_MARK_AS_STARTED = String.format("SET @update_id := 0;UPDATE %s SET %s = '%s', %s = (SELECT @update_id := %s) WHERE %s = '%s' ORDER BY %s DESC, %s, %s LIMIT 1;SELECT @update_id;", IDENTIFIER_TABLE_VIDEO_QUEUE, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_STATE, QueuedVideoState.STARTED.name(), IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_ID, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_ID, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_STATE, QueuedVideoState.QUEUED.name(), IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_PRIORITY, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_REQUESTED, IDENTIFIER_TABLE_VIDEO_QUEUE_COLUMN_ID);
    //
    // // Inserts
    // Table: Channel
    /**
